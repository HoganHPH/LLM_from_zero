...
def preprocess_function(examples):
    inputs = [prefix + doc for doc in examples['text']] # concat prompt and text
    model_inputs = tokenizer(inputs, max_length=1024, truncation=True) # inputs training
    
    labels = tokenizer(text_target=examples['summary'], max_length=1024, truncation=True)
    model_inputs['labels'] = labels['input_ids'] # true labels used in training
...
def compute_metrics(eval_pred):
    ...
    result = rouge.compute(predictions=decoded_preds, refereces=decoded_labels, use_stemmer=True)
